...
export const getLegalServices = async (req, res) => {
    try {
        // ✅ Acepta tanto 'offset' como 'page'
        const offsetParam = req.query.offset ? parseInt(req.query.offset) : null;
...
export const getLegalServiceById = async (req, res) => {
    try {
        const { id } = req.params;
...
export const updateLegalService = async (req, res) => {
    try {
        const { id } = req.params;
        const { name, id_category, price, duration, description, status } = req.body;
        const service = await LegalService.findByPk(id);
...
export const deleteLegalService = async (req, res) => {
    try {
        const { id } = req.params;
        const service = await LegalService.findByPk(id);
        if (!service) {
...
//# sourceMappingURL=legalServiceHandler.js.map